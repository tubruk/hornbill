...
export function Sidebar({
  ...
  const currentTitle = ROUTE_TITLES[location.pathname] ?? ROUTE_TITLES["/"];
  ...
  return (
    <aside className="w-full md:w-64 bg-background-warm border-b md:border-b-0 md:border-r border-border-warm flex flex-col z-20 shrink-0 select-none">
      ...
      <div className="p-3.5 md:p-5 border-b border-border-warm flex items-center justify-between md:block">

        {/* Brand mark */}
        <div className="flex items-center gap-3">
          <img src={logo} alt="Hornbill Logo" className="w-8 h-8 md:w-9 md:h-9 object-contain rounded-sm shadow-sm shrink-0" />
          <div>
            <h1 className="font-display font-bold text-[18px] md:text-[20px] text-text-primary tracking-tight leading-none">
              <span className="md:hidden">{currentTitle}</span>
              <span className="hidden md:inline">Hornbill</span>
            </h1>
            <span className="text-[11px] font-semibold text-neutral-muted uppercase tracking-wider hidden md:block mt-0.5">
              Bill Tracker
            </span>
          </div>
        </div>
      ...
      <div className="flex md:hidden fixed bottom-0 left-0 right-0 items-center justify-around border-t border-border-warm pt-2.5 pb-4 px-4 bg-surface-warm shadow-[0_-4px_12px_rgba(28,25,23,0.05)] z-30">
        {NAV_ITEMS.map(({ to, label, icon: Icon }) => {
          const isActive = to === "/" ? location.pathname === "/" : location.pathname.startsWith(to);
          return (
            <Link
