...
export type EnrichedPayment = Payment & { bill: { name: string } | null };
...
export function usePayments(
  ...
  return useQuery<EnrichedPayment[]>({
    ...
    queryFn: async () => {
      ...
      // Filter to this account's bills and enrich with bill names
      return all
        .filter((p) => billMap.has(p.bill_id))
        .map((p) => {
          const bill = billMap.get(p.bill_id);
          return { ...p, bill: bill ? { name: bill.name } : null };
        });
    },
